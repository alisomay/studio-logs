...
21 | EIN4 <--> Ensemble HI/Z In 4
22 | EIN3 <--> Ensemble HI/Z In 3
23 | FREE
24 | FREE
```
...
```
Ensemble (Physical) HI/Z IN 1 --> Apollo ADAT IN 1 (IN 9)
Ensemble (Physical) HI/Z IN 2 --> Apollo ADAT IN 2 (IN 10)
Ensemble (Physical) HI/Z IN 3 --> Apollo ADAT IN 3 (IN 11)
Ensemble (Physical) HI/Z IN 4 --> Apollo ADAT IN 4 (IN 12)
...
This enables for us to extend our output count `10` to `18` which will give us more patching capabilites using outboard, external effects etc.
...
Apogee Ensemble Firewire is a discontinued product but it has top notch converters and a professional audio interface for its time.

The downside is it is not working with the latest mac os anymore die to Apple dropping firewire support from CoreAudio. The only way to configure the audio interface is to use an older version of mac os. I have an old computer which can accomplish this task and in my next visit I'll revise the configuration.

The standalone mode mitigates this situation partially. Because we can save a routing to the interface and use it without a computer, also we can control some functions from the front facing panel.

It is important that ensemble output volume is always in full position because then it is transparent. Digital audio interfaces usually work by attenuation but not amplification. So volume at 0db means signal is not amplified digitally.

The left knob controls 4 pre amp's input levels. Ensemble has also 4 mic inputs in the back of it which are not connected at the moment. All 4 preamps are configured as HI/Z at the moment. The other 4 inputs are line level inputs with no preamps.

Temporarily the routing of the Ensemble is broken but it will be fixed within a week, please take the previous routing documentation in this document as the source of truth.
